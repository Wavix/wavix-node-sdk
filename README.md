...
The Wavix TypeScript library provides convenient access to the Wavix APIs from TypeScript.
...
A full reference for this library is available [here](https://github.com/wavix/wavix-typescript/blob/HEAD/./reference.md).
...
```typescript
import { WavixClient, WavixEnvironment } from "@wavix/sdk";
...
```typescript
const { data, rawResponse } = await client.apiKeys.create(...).withRawResponse();
